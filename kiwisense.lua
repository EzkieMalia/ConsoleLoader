...
repeat task.wait() until (os.time() - game.Players.LocalPlayer:GetAttribute("JoinTime") >= 35)

local Hyphon_Bypassed = false
local Start = tick()
local function HyphonBypass()
    local HandshakeFunctions = {}
    for Index, Object in next, getgc(false) do
        if (typeof(Object) == "function" and islclosure(Object)) then 
            local DebugInfo = debug.info(Object, "s")
            if (DebugInfo:match("PlayerGui") and DebugInfo:match("%x+")) then
                local Constants = debug.getconstants(Object)
                local UpValues = debug.getupvalues(Object)

                for Index, Constant in next, Constants do
                    if tostring(Constant):match("19996") or tostring(Constant):match("16379") then
                        --print(getfunctionhash(Object))
                    end
                end
                if (#UpValues == 20 and #Constants > 140 and #Constants < 150) then
                    table.insert(HandshakeFunctions, Object)
                end
            end
        end
    end
    if (#HandshakeFunctions ~= 0 and #HandshakeFunctions >= 22) then
        for Index, Function in next, HandshakeFunctions do
            if Index == 2 then
                local Old
        ...
        Hyphon_Bypassed = true
    else
        game.Players.LocalPlayer:Kick("Code 404 | report in server")
        return
    end
end
HyphonBypass()

if Hyphon_Bypassed == true then
    warn("Anticheat Bypass Successfully Ran. | Time Taken: " .. string.sub(tostring(tick() - Start), 1, 6) .. " seconds")
else
    game.Players.LocalPlayer:Kick("Code 500 | report in server")
end

local hitboxExpanderEnabled = false
local hitboxExpanderSize = 1
local hitboxOriginalSize = nil
local hitboxTransparency = 0

aimbot = false
aimbotRangeEnabled = false
aimbotRange = 500
AimbotKeyHeldDown = false
...
hitboxOriginalSize = LocalPlayer.Character:WaitForChild("Head").Size

task.spawn(function()
    while task.wait(.025) do
        for i,v in next, Players:GetPlayers() do
            if v.Character and v.Character:FindFirstChild("Head") and v ~= LocalPlayer and not whitelisted[v.Name] then
                local head = v.Character:FindFirstChild("Head")
                head.Transparency = hitboxTransparency
                if hitboxExpanderEnabled == true then
                    head.CanCollide = false
                    head.Massless = true
                    head.Size = hitboxOriginalSize * hitboxExpanderSize
                else
                    if head.Size == hitboxOriginalSize then
                        head.CanCollide = true
                        head.Massless = false
                    end
                    head.Size = hitboxOriginalSize
                end
            end
        end
    end
...
Players.PlayerAdded:Connect(function(plr)
    if not whitelisted[plr.Name] and plr ~= game.Players.LocalPlayer then 
        local char = plr.Character or plr.CharacterAdded:Wait()
        local head = char:WaitForChild("Head")

        if hitboxExpanderEnabled == true then 
            head.Size = head.Size * hitboxExpanderSize
...
local GhostMachete = false
local ghostmacheteDebounce = false

local function cleanupDiedConnection()
...
local function getNearestPlayerToCursor(maxDistance)
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if player ~= LocalPlayer
			...
			and player.Character.Humanoid.Health > aimbotHealthCheck then

			local hrp = player.Character.HumanoidRootPart
			local localChar = LocalPlayer.Character

			if localChar and localChar:FindFirstChild("HumanoidRootPart") then
				if (hrp.Position - localChar.HumanoidRootPart.Position).Magnitude > maxDistance then
					continue
				end
			end
...
local function isTargetValid(player)
    ...
    if whitelisted[player.Name] then return false end
	...
	local localChar = LocalPlayer.Character
	if localChar and localChar:FindFirstChild("HumanoidRootPart") then
		if (hrp.Position - localChar.HumanoidRootPart.Position).Magnitude > aimbotRange and aimbotRangeEnabled then
			return false
		end
	end
	...
	if lockedTarget and lockedTarget.Character then
		...
		if humanoid then
			diedConnection = humanoid.Died:Connect(function()
				lockTarget(getNearestPlayerToCursor(aimbotRange))
			end)
		end
	end
end

function stopAiming(connection)
	if connection then
...
local connection = RunService.RenderStepped:Connect(function()
    ...
    if not AimbotKeyHeldDown then stopAiming() return end
    ...
    local head = lockedTarget.Character:FindFirstChild(aimbotHitPart)
    if not head then return end

    local targetPos, onScreen = Camera:WorldToViewportPoint(head.Position)
    if not onScreen then return end

    local mousePos = UserInputService:GetMouseLocation()
    mousemoverel(
        (targetPos.X - mousePos.X) * aimbotSmoothness,
        (targetPos.Y - mousePos.Y) * aimbotSmoothness
    )
end)
...
local function silentaim()
    local silentAimShowHealthESP = false
    local silentAimMaxRange = 50000
    ...
    local inflict = rs:WaitForChild("RemoteEvents"):WaitForChild("InflictTarget")
    ...
    local function canSeeTarget(origin, targetPos)
        ...
        if result then
            return isCharacterPart(result.Instance)
        end
    ...
    local function target()
        ...
        local best, dist = nil, silentAimFOV + 5 -- small buffer for edge issue
        local camPos = cam.CFrame.Position

        for _,p in ipairs(players:GetPlayers()) do
            if p ~= me and not whitelisted[p.Name] then
                local c = p.Character
                local h = c and c:FindFirstChildOfClass("Humanoid")
                local part = c and c:FindFirstChild(silentAimHitPart)

                if h and part and h.Health > 0 then
                    local distance = (camPos - part.Position).Magnitude
                    
                    if distance <= silentAimMaxRange then
                        local s, on = cam:WorldToViewportPoint(part.Position)
                        if on then
                            local d = (Vector2.new(s.X,s.Y) - m).Magnitude
                            if d < dist then
                                if canSeeTarget(camPos, part.Position) then
                                    best = part
                                    dist = d
                                end
                            end
    ...
    runService.RenderStepped:Connect(function()
        ...
        if silentAimShowLine and currentTarget then
            local screenPos, onScreen = cam:WorldToViewportPoint(currentTarget.Position)
            
            if onScreen then
                tracerLine.From = Vector2.new(centerX, centerY)
    ...
    local castblack = find("CastBlacklist")
    local castwhite = find("CastWhitelist")
    ...
    local oldCast
    oldCast = hookfunction(castblack, newcclosure(function(...)
        if not chance(silentAimHitChance) then
            return oldCast(...)
        end

        local t = currentTarget -- ✅ FIXED

        if t then
            local a = {...}
            a[2] = t.Position - a[1]
            a[3] = {t.Parent}
            return castwhite(unpack(a))
        end
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if not busy and method == "FireServer" and self == inflict then
            busy = true
            ...
            for i = 1, 8 do
                local newArgs = table.clone(args)
                if typeof(newArgs[last]) == "number" then
                    newArgs[last] = newArgs[last] + (i * 0.001)
                end
                task.wait()
                old(self, unpack(newArgs))
            end
...
NeverloseSound.Parent = workspace.Camera
NeverloseSound.SoundId = "rbxassetid://97643101798871"
NeverloseSound.Volume = 10
...
local Options, MiscOptions do
    ...
    MiscOptions = {
        ...
        ["Box Fill"] = false; 
        ["Box Fill 1"] = { Color = rgb(0, 255, 255), Transparency = 0.5 };
        ["Box Fill 2"] = { Color = rgb(0, 255, 255), Transparency = 0.5 };
        ["Box Fill Rotation"] = 0;
...
local Library do
    ...
    local Themes = {
       ...
       ["Velvet"] = {
            ["Background"] = FromRGB(16, 18, 21),
            ["Inline"] = FromRGB(23, 23, 23),
            ["Shadow"] = FromRGB(0, 0, 0),
            ["Text"] = FromRGB(255, 255, 255),
            ["Image"] = FromRGB(255, 0, 0),
            ["Dark Gradient"] = FromRGB(179, 175, 175),
            ["Inactive Text"] = FromRGB(185, 185, 185),
            ["Element"] = FromRGB(37, 38, 39),
            ["Accent"] = FromRGB(255, 0, 0),
            ["Border"] = FromRGB(24, 24, 24)
        },
    }

    Library.Theme = TableClone(Themes["Velvet"])
    ...
    do -- Element functions
        ...
        Library.ChatSystem = function(self, Data)
            ...
            function GlobalChat:SetVisibility(Bool)
                ...
                local parent = Bool and Data.MainFrame.Instance or Library.UnusedHolder.Instance
                Items["Chat_System"].Instance.Parent = parent
            end
        ...
        Library.Window = function(self, Data)
            ...
            local Window = {
                ...
                Logo = Data.Logo or Data.logo or "131289678336916",
            ...
            Dropdown.Callback = function(Value) -- horrible code ik
                if Playerlist.Player then
                    ...
                    if Value == "Neutral" then
                        ...
                        whitelisted[Playerlist.Players[Playerlist.Player.Name]] = nil
                    ...
                    elseif Value == "Friendly" then
                        ...
                        whitelisted[Playerlist.Players[Playerlist.Player.Name]] = true
...
local increaseWalkspeed = false
local walkspeedAddifier = 0

local MovementController = require(game:GetService("Players").LocalPlayer.PlayerScripts["Client.Initializer"].Modules.MovementController)

local infiniteStamina = false
local originalMovespeeds = {
    ["normalSpeed"] = MovementController["normalSpeed"],
    ["crouchNormalSpeed"] = MovementController["crouchNormalSpeed"],
    ["sprintSpeed"] = MovementController["sprintSpeed"],
    ["crouchSpeed"] = MovementController["crouchSpeed"],
}
...
task.spawn(function()
    while task.wait(.05) do
        ...
        if increaseWalkspeed == true then
            for i, speed in pairs(originalMovespeeds) do
                MovementController[i] = speed + walkspeedAddifier
            end
        else
            for i, speed in pairs(originalMovespeeds) do
                MovementController[i] = speed
            end 
        end
    end
end)
...
do
    local Window = Library:Window({
        Name = "Velvet | South Bronx: The Trenches ❗",
        Version = "v1.00",
        Logo = "131289678336916",--"135215559087473",
        FadeSpeed = 0.25,
    ...
    ChatSystem = Library:ChatSystem({
        MainFrame = Window.Items["MainFrame"] -- keep this
    })

    local Watermark = Library:Watermark("Velvet", "131289678336916") -- 135215559087473
    ...
    do
        ...
        do 
            ...
            local walkspeedSection = subs["Main"]:Section({Name = "Main", Icon = "103174889897193", Side = 1})
            local console = subs["Main"]:Section({Name = "Console", Icon = "103174889897193", Side = 2})
            local higuysomni = subs["Main"]:Section({Name = "Other", Icon = "136623465713368", Side = 1})

            local Button = console:Button({
                    Name = "Console Servers", 
                    Callback = function()
                        pcall(function(p1_4, a_4, b_4, c_4)
                            queue_on_teleport[[
                                ...
                                task.spawn(function()
                                    while true do
                                        pcall(function()
                                            local detectPlatform = ReplicatedStorage:FindFirstChild("DetectPlatform")
                                            if detectPlatform and detectPlatform:IsA("RemoteFunction") then
                                                detectPlatform.OnClientInvoke = function()
                                                    return true
                                                end
                                            end
                                        end)
                        ...
                        pcall(function(p1_5, p2_5, p3_5, a_5, b_5, c_5)
                            game:GetService("TeleportService"):Teleport(10179538382, LocalPlayer_1);
                        end);
                    end
                })

            local walkspeedtoggle = walkspeedSection:Toggle({
                Name = "Modify Walkspeed",
                Flag = "toggle",
                Default = false,
                Callback = function(Value)
                    increaseWalkspeed = Value
                end
            })

            local WalkSpeedSlider = walkspeedSection:Slider({
                Name = "Walkspeed Addifier", 
                Flag = "Slider", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                Min = 0, 
                Default = 0, 
                Max = 5, 
                ...
                Decimals = 1, 
                Callback = function(Value)
                    walkspeedAddifier = Value
                end
            })

            local staminatoggle = walkspeedSection:Toggle({
            ...
            local ghostmachetenable = higuysomni:Toggle({
                Name = "Ghost Machete",
                Flag = "enabled",
                Default = false,
                Callback = function(Value)
                    GhostMacheteEnabled = Value
                end
            })

            local Label = higuysomni:Label("Keybind", "Left")

            local holding = false
            local ghostmacheteDebounce = false

            local keybindobj = Label:Keybind({
                Name = "Machete Keybind",
                Flag = "Machete Keybind",
                ...
                Callback = function(Value)
                    ...
                    task.spawn(function()
                        while holding do
                            if not GhostMacheteEnabled then
                                break
                            end

                            if not ghostmacheteDebounce then
                                print("sending")
                                ghostmacheteDebounce = true

                                local meleeRemote = game.ReplicatedStorage
                                    :WaitForChild("RemoteEvents")
                                    :WaitForChild("Melee")

                                meleeRemote:FireServer(buffer.fromstring("\a"))

                                task.wait(0.3)
                                ghostmacheteDebounce = false
                            else
                                task.wait()
                            end
                        end
            ...
            local Dropdown = higuysomni:Dropdown({
                Name = "Keybind Mode",
                Flag = "Dropdown",
                Items = {"hold"},
                Default = "hold",
                ...
                Multi = false,
                Callback = function(Value)
                    keybindobj.Mode = Value
                end
            })

            Dropdown:AddOption("toggle")
        end
        ...
        do -- Combat
            local Subpages = {
                ...
                ["SilentAim"] = Pages["Combat"]:SubPage({
                    Name = "Silent Aim", 
                    Icon = "126028986879491", 
                    Columns = 2
                }),
                ["GunMods"] = Pages["Combat"]:SubPage({
                    Name = "Gun Mods",
                    Icon = "137300573942266",
                    Columns = 2,
                }),
                ["HitboxExpander"] = Pages["Combat"]:SubPage({
            ...
            do
                local GeneralSection = Subpages["SilentAim"]:Section({Name = "Silent Aim", Icon = "103174889897193", Side = 1})
                local GeneralSection2 = Subpages["Aimbot"]:Section({Name = "Aimbot", Icon = "103174889897193", Side = 1})
                local GeneralSection3 = Subpages["HitboxExpander"]:Section({Name = "Hitbox Expander", Icon = "109463522861706", Side = 1})
                local GeneralSection4 = Subpages["GunMods"]:Section({Name = "Gun Modifications", Icon = "126028986879491", Side = 1})
                local GeneralSection5 = Subpages["GunMods"]:Section({Name = "Sliders", Icon = "111178525804834", Side = 2})

                do
                    local silentaimtoggle = GeneralSection:Toggle({
                        Name = "Silent Aim",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local hitchanceslider = GeneralSection:Slider({
                        Name = "Hit Chance", 
                        Flag = "HitChance", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                        Min = 0, 
                        Default = 100, 
                    ...
                    local wallbangtoggle = GeneralSection:Toggle({
                        Name = "Wall Bang",
                        Flag = "enabled",
                        Default = true,
                        Callback = function(Value)
                    ...
                    local silentAimDropdown = GeneralSection:Dropdown({
                        Name = "Silent Aim Hit Parts",
                        Flag = "Silent Aim Hit Parts",
                        Items = {"Head"},
                        Default = "Head",
                        MaxSize = 155,
                        Multi = false,
                    ...
                    silentAimDropdown:AddOption("UpperTorso")
                    silentAimDropdown:AddOption("LowerTorso")

                    GeneralSection:Label("Field Of View", "Left")
                    ...
                    GeneralSection:Label("FOV Color", "Left"):Colorpicker({
                        Name = "Colorpicker",
                        Flag = "Colorpicker23",
                    ...
                    local aimtoggle = GeneralSection2:Toggle({
                        Name = "Aimbot",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local Label = GeneralSection2:Label("Keybind", "Left")
                    local keybindobj = Label:Keybind({
                        Name = "Aimbot Keybind",
                        Flag = "Aimbot Keybind",
                        Default = Enum.KeyCode.X,
                        Mode = "hold",
                        Callback = function(Value)
                            AimbotKeyHeldDown = Value
                        end
                    })

                    local Dropdown = GeneralSection2:Dropdown({
                        Name = "Aimbot Mode",
                        Flag = "Dropdown",
                        Items = {"hold"},
                        Default = "hold",
                    ...
                    local aimDropdown = GeneralSection2:Dropdown({
                        Name = "Aimbot Parts",
                        Flag = "Aimbot Parts",
                        Items = {"Head"},
                        Default = "Head",
                    ...
                    aimDropdown:AddOption("UpperTorso")
                    aimDropdown:AddOption("LowerTorso")

                    local aimdisttoggle = GeneralSection2:Toggle({
                        Name = "Enable Aimbot Range",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                            aimbotRangeEnabled = Value
                        end
                    })

                    local MaxDistance = GeneralSection2:Slider({
                        Name = "Aimbot Range", 
                        Flag = "Aimbot Range", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                        Min = 0, 
                        Default = 500, 
                    ...
                    local aimsmooth = GeneralSection2:Slider({
                    ...
                    local aimHealthcheck = GeneralSection2:Slider({
                        Name = "Aimbot Health Check", 
                        Flag = "Aimbot Health Check", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                        Min = 0, 
                        Default = 0, 
                        Max = 100, 
                        Suffix = "", -- % 
                    ...
                    local hitboxtoggle = GeneralSection3:Toggle({
                        Name = "Hitbox Expander",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local hitboxSlider = GeneralSection3:Slider({
                        Name = "Hitbox Size", 
                        Flag = "Slider", 
                        Min = 1, 
                        Default = 1, 
                    ...
                    --[[local infAmmo = GeneralSection4:Toggle({
                        Name = "Infinite Ammo",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local InstantEquip = GeneralSection4:Toggle({
                        Name = "Instant Equip",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local ModifyRange = GeneralSection4:Toggle({
                        Name = "Modify Range",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local AutoMod = GeneralSection4:Toggle({
                        Name = "Rapid Fire",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local InstantReload = GeneralSection4:Toggle({
                        Name = "Instant Reload",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local noRecoil = GeneralSection4:Toggle({
                        Name = "No Recoil",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local noSpread = GeneralSection4:Toggle({
                        Name = "No Spread",
                        Flag = "enabled",
                        Default = false,
                        Callback = function(Value)
                    ...
                    local modifyFirerate = GeneralSection4:Toggle({
                    ...
                    GeneralSection5:Label("100% is the default FireRate.", "Left")
                    GeneralSection5:Label("Anything under 65% is not recommended for PC Servers.", "Left")
                    GeneralSection5:Label("Enable BEFORE equipping a gun. If you enable it after equipping the gun, it will not work.", "Left")
                    local firerateSlider = GeneralSection5:Slider({
                        Name = "FireRate Slider", 
                        Flag = "firerateSlider", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                        Min = 0, 
                        Default = 100, 
                        Max = 100, 
                    ...
                    local rangeslider = GeneralSection5:Slider({
                        Name = "Range Slider", 
                        Flag = "rangeslider", -- 🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫🇦🇫
                        Min = 0, 
                        Default = 500, 
                    ...
                    --[[local Button = GeneralSection2:Button({
                        Name = "CONSOLE SERVERS", 
                        Callback = function()
                            print("Pressed")
                        end
        ...
        do -- Visuals
            ...
            local NameMods = Pages["Visuals"]:Section({Name = "Name Modifications", Side = 2, Icon = "115907015044719"})
            local Textbox = NameMods:Textbox({
                Name = "NameTag",
                Flag = "nametag",
                Placeholder = "discord.gg/velvetsb",
                Default = ""..game.Players.LocalPlayer:GetAttribute("FullName"),
                Callback = function(Value)
                    local char = game.Players.LocalPlayer.Character or game.Players.LocalPlayer.CharacterAdded:Wait()
                    local oldRank = char.Head:FindFirstChild("NameTag")
                    local newRank = char.Head:FindFirstChild("SpoofedNameTag") or oldRank:Clone()
                    newRank.Parent = char.Head
                    newRank.Enabled = true
                    newRank.Name = "SpoofedNameTag"
                    newRank.MainFrame.NameLabel.Text = Value
                    oldRank.Enabled = false
                end
            })
            local Textbox2 = NameMods:Textbox({
                Name = "RankTag",
                Flag = "ranktag",
                Placeholder = "discord.gg/velvetsb",
                ...
                Callback = function(Value)
                    local char = game.Players.LocalPlayer.Character or game.Players.LocalPlayer.CharacterAdded:Wait()
                    local oldRank = char.Head:FindFirstChild("RankTag")
                    local newRank = char.Head:FindFirstChild("SpoofedRankTag") or oldRank:Clone()
                    newRank.Parent = char.Head
                    newRank.Enabled = true
                    newRank.Name = "SpoofedRankTag"
                    newRank.MainFrame.NameLabel.Text = Value
                    oldRank.Enabled = false
                end
            })
        ...
        do -- Settings
            ...
            do -- Theming
                ...
                ThemingProfiles:Dropdown({
                    Name = "preset themes",
                    Items = { "Preset", "Halloween", "Aqua", "One Tap", "Velvet" },
                    Default = "Velvet",
                    Multi = false,
                    Callback = function(Value)
...
Library:Notification({
    Name = "Velvet | Script Loader",
    Description = "Script loaded in: " .. string.format("%.4f", os.clock() - LoadingTick) .. " seconds",
    Duration = 5
})
...
local function hookBackpack(backpack)
    backpack.ChildAdded:Connect(function(child)
        for i,v in next, backpack:GetChildren() do
            ...
            local setting = child:FindFirstChild("Setting")
            if not setting or not setting:IsA("ModuleScript") then
                continue
            end

            local ok, config = pcall(require, setting)
            ...
            if config.FireRate then
                local s, e = pcall(function()
                    if not originalFireRates[child] then
                        originalFireRates[child] = config.FireRate
                    end

                    if modifyFirerate then
                        local original = originalFireRates[child]
                        local percent = firerate / 100
                        config.FireRate = original * percent
                    end
                    ...
                    if InstantEquip then 
                        config.EquipTime = 0
                        config.EquippingTime = 0
                        config.EquippedAnimationSpeed = 100
                    end
                    ...
                    --[[if InfiniteAmmo then
                        ...
                        if ammo then
                            ammo.Value = config.MaxAmmo
                        end
                    end]]

                    if rapidFire then
                        if config.Auto ~= nil and config.Auto == false then
                            config.Auto = true
                        end
                    end

                    if InstantReload then 
                        if config.ReloadTime ~= nil and config.ReloadAnimationSpeed ~= nil then
                            config.ReloadTime = 0
                            config.ReloadAnimationSpeed = 100
                        end
                    end
                end)
            end
...
mt.__namecall = newcclosure(function(self,...)
    ...
    if method == "FireServer" and tostring(self) == "Shoot" then
        ...
        if typeof(payload) == "buffer" then
            local auto = buffer.readu8(payload,7)

            if auto ~= 0 then
                buffer.writeu8(payload,7,0)
            end
        end
    end
    return old(self,...)
